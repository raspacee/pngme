...
impl Png {
    ...
    pub fn as_bytes(&self) -> Vec<u8> {
        let mut buf: Vec<u8> = self.header.iter().cloned().collect();
    ...
    pub fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
        for i in 0..self.chunks().len() {
            if self.chunks[i].chunk_type().bytes() == chunk_type.as_bytes() {
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Self> {
        ...
        if Png::STANDARD_HEADER == header {
            ...
            Err(Box::new(PngError::InvalidHeaderBytes))
        }
    } 
}
...
impl fmt::Display for PngError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::InvalidHeaderBytes => {
                write!(f, "First 8 bytes don't match with PNG file headers")
            }
            Self::ChunkNotFound => {
                write!(f, "Chunk not found in the PNG")
            }
        }
    }
}
